...
class Helper:
    url :str = "https://www.slickcharts.com/nasdaq100"
    #url: str  = "http://127.0.0.1:8080/nq100.html"
    body: str = None

    def getHeaders(self):
        return {"User-Agent":ua[random.randint(0,len(ua)-1)]}

    def getRequest(self):
        r = requests.get(self.url, headers=self.getHeaders())
        self.body = r.text

    def parse(self):
        soup = BeautifulSoup(self.body, "html.parser")
        table = soup.find("table", {"class", "table table-hover table-borderless table-sm"})
        tableBody :bs4.element.Tag = table.tbody
        rows : bs4.element.ResultSet = tableBody.findAll("tr")

        l = []
        for x in rows:
            v : bs4.element.Tag =x
            l.append(v.find_all("a")[1].text)
